...
def getCoordinationEdits(row):
    ...
    this_ww = date_to_ww(links[0]["cpl_link_date"])
    ...
    query = "SELECT * FROM coord_project_links WHERE cpl_page_id IN (%s) AND cpl_link_date >= '%s' GROUP BY cpl_link_page_text, spl_link_date ORDER BY cpl_link_date ASC" % (",".join(map(str, project_page_ids + project_page_talk_ids)), row["p_created"])
    lc = ldb.execute(query)
    links = lc.fetchall()
...
def main():
    # First, grab the talk page ids of all projects
